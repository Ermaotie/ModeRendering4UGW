...
S_o.Faces = mesh_list;
S_o.FaceColor = face_color;
S_o.EdgeColor = edge_color;
S_o.LineWidth = line_width;
...
S_d.Faces = mesh_list;
S_d.FaceColor = face_color;
S_d.EdgeColor = edge_color;
S_d.LineWidth = line_width;

% 设置侧面S_trans
S_trans.Vertices = trans_nodes_xyz;
S_trans.Faces = trans_ele;
S_trans.FaceColor = face_color;
S_trans.EdgeColor = edge_color;
S_trans.LineWidth = line_width;
...
for t = time
    ...
    % displacement_o = real(sqrt(sum(nodes_displacement_o.*nodes_displacement_o,2)));
    % ele_mean_o = mean(displacement_o(mesh_list),2);
    % 
    % displacement_d = real(sqrt(sum(nodes_displacement_d.*nodes_displacement_d,2)));
    % ele_mean_d = mean(displacement_d(mesh_list),2);
    % 
    % displacement_trans = real(sqrt(sum(nodes_displacement_trans.*nodes_displacement_trans,2)));
    ...
    % pause(0.05);
    % colorbar;
    exportgraphics(gcf,'image_trail_400k/zhenxing/m'+string(m)+'-Cg'+string(cg)+'-f'+string(f)+'kHz'+'-3d.gif','Append',true);
end
...
xlim([0 500]);
ylim([0 5000]);
xlabel('频率(kHz)');ylabel('群速度Cph(m/s)');
saveas(figure(3),'image_trail_400k/Cg/m'+string(m)+'-Cg'+string(cg)+'-f'+string(f)+'kHz'+'-3d.png');
saveas(figure(3),'image_trail_400k/Cg/m'+string(m)+'-Cg'+string(cg)+'-f'+string(f)+'kHz'+'-3d.fig');
